...
int controller_addEmployee(LinkedList* pArrayListEmployee)
{
    ...
    if(pArrayListEmployee!=NULL && empleado!=NULL)
    {
        ...
        getEntero(&id, "\nIngrese Id: ", "Error. Ingrese un Id valido.", 1, 25000, 3);
        existeId = controller_existeId(pArrayListEmployee, id);

        while(existeId>=0 && reintentos>0)
        {
    ...
    if(reintentos>0 && index>=0)
    {
        ...
        for(i=0; i<tamanioArray; i++)
        {
            ...
            if(empleado!=NULL && idEmpleado==idEmpleadoAModificar)
            {
                ...
                {
                case 1:
                    if(getEntero(&nuevoValorEntero, "\nIngrese nuevo Id: ", "\n***** Error. Ingrese un numero valido. ******", 1, 100000, 3))
                    {
                        if(controller_existeId(pArrayListEmployee, nuevoValorEntero)>=0)
                        {
                            printf("\n***** Error. Ese Id ya lo tiene otro empleado ******\n\n");
                            system("pause");
                        }
                        else
                        {
                            ...
                            if(confirma == 's' && employee_setId(empleado, nuevoValorEntero))
                            {
...
int controller_menu(void)
{
    ...
    getEntero(&opcion, "\n\nIngrese una opcion: ", "\nError. Ingrese una opcion valida", 1, 10, 10);
